...
##############################################################################
{
    ...
    'summary': 'A per-company financial KPI dashboard for Odoo 18 Community that puts cash position, AR and AP aging, and period revenue, expense and net on one screen, every tile computed with single parameterised SQL passes off the same engine as the suite reports so figures match the trial balance to the cent. Odoo 18 Community financial dashboard, accounting KPI dashboard, cash position dashboard, accounts receivable and payable aging, period P and L, CFO finance dashboard, cash flow sparkline trend, drill-down accounting dashboard, multi-company KPI dashboard.',
    'description': """The single screen that summarises a company's financial state and links straight to the detail behind every number. Six core tiles always show: cash position, receivables (total and overdue, plus the age of the oldest open item), payables (total and overdue), and period revenue, expense and net. Up to eight more tiles light up automatically when the matching suite modules are installed, covering pending approvals, active collections cases, budgets in overrun, credit-limit signals, SEPA mandate dormancy, and year-end, period-close and FX revaluation runs.

Every KPI is computed with single parameterised SQL passes per tile (no per-row ORM access), using the same MoveLineQuery engine that drives the suite's trial balance and financial reports. That shared engine is why the dashboard figures reconcile to the trial balance to the cent rather than approximating them.
