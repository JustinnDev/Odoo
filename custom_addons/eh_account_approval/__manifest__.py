...
Amount-band policies. One policy per company and document type, with ordered rules. An amount band selects the single approver group required for that step, and first-match rule routing picks the band. A bill below one threshold might need only a manager; a bill above a higher one walks manager, then director, then CFO.

Multi-step ordered approvals. Each step is bound to exactly one approver group and any member of that group can sign off to advance the step. Steps run top-to-bottom in the policy's sequence order, with the order pinned to an explicit sequence column rather than the security-group primary key, so creating or deleting unrelated groups never silently reorders an in-flight chain.
