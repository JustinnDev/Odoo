...
Every export writes an audit record stamped with the generating user, timestamp, message ID, transaction count, control sum and a SHA-256 file hash. Regenerating a batch supersedes the prior file rather than deleting it, and the stored hash lets an auditor spot a duplicate generation. State moves from generated to downloaded when you pull the file.

Built directly from the public ISO 20022 specification. Original work, not a derivative. Pairs with the batch payment module: build the batch, generate the SEPA file, upload to the bank.""",
    'author': 'ERP Heritage',
