...
##############################################################################
{
    ...
    'summary': 'Shared reporting engine for the ERP Heritage Odoo 18 Community accounting suite, providing a reproducible report-execution audit log and a move-version result cache. Append-only audit log of every report render with user, timestamp, the full options snapshot, runtime, and a SHA-256 result hash for XLSX and PDF output, plus a multi-company analytic-aware parameterised SQL builder with no raw user input in SQL, a three-tier User/Manager/read-only Auditor privilege model, and a currency-aware XLSX writer. Odoo 18 Community accounting reporting engine, reproducible financial report audit trail, move-version report cache invalidation, multi-company parameterised SQL builder, append-only audit log, read-only auditor role, SHA-256 report hash compliance, currency-aware XLSX export, analytic-aware journal item aggregation.',
    'description': """The shared engine that powers the ERP Heritage accounting modules for Odoo 18 Community. It auto-installs as a dependency of any ERP Heritage accounting module, so you do not need to install it by hand.
